...
from lzstr.Exceptions import LZStringDecompressionException


class LZStringTests(unittest.TestCase):
    def setUp(self):
        self._test_vectors = json.loads(
            pkgutil.get_data("lzstr.tests", "test_vectors.json")
        )
    ...
    def test_decompress_circuitjs(self):
        bs = BitString.from_url_component(
            "CQAgjCAMB0l3BWc0FwCwCY0HYEA4cEMElURTJyBTAWjDACgwE0QMs21KBmANj06VKGKOSZl2rMGlZ8B01sNEIGAGXAZ5vSnkphtbUQDMAhgBsAzlXJQ1GgZJC62HEZVOXrSSAwDu9lykDRx9-fWEOcIDQ8AMwTUDov1iI1kcQ5PitPQBOESiYsDyU8GLiXlswsoQK9JrK0vzgyIMfAFkQOXAZEDR9brS2FAYOrqxKPtquQwxhoA"
        )
        self.assertEqual(
            LZStringDecompressor(bs).decompress(),
            b"$ 1 0.000005 1.500424758475255 50 5 50 5e-11\n154 224 240 368 240 0 2 0 5\n150 224 144 368 144 0 2 0 5\nL 128 160 80 160 2 0 false 5 0\nL 128 224 80 224 2 0 false 5 0\nw 128 224 160 224 0\nw 160 224 160 128 0\nw 160 128 224 128 0\nw 160 224 224 224 0\nw 128 160 192 160 0\nw 192 160 192 256 0\nw 192 256 224 256 0\nw 192 160 224 160 0\nM 368 144 416 144 2 2.5\nM 368 240 416 240 2 2.5\n",
        )
    ...
    def test_run_vectors_base64_decompression(self):
        for test_vector in self._test_vectors:
            ...
            self.assertEqual(
                LZStringDecompressor(bs).decompress(), test_vector["uncompressed"]
            )
    ...
    def test_base64_fragment_via_url_component_raises_not_hangs(self):
        # Regression test: decompress_from_url_component must raise LZStringDecompressionException
        # (not hang) when given a base64-encoded fragment (which uses '/' not in the URI-safe
        # alphabet).
        # This specific fragment is known to contain '/'.
        b64_fragment = "EQJgTgtgNAZABgDgOIgDYHkQCsDEBlCAdQF4AGYgCgE8AzASAA8BOJgbQE1iBqAJQFgAQgBUA/AC0AugDUAlABYIAZwDsAOwCGYYAEEg"
        self.assertIn("/", b64_fragment)
        with self.assertRaises(LZStringDecompressionException):
            LZStringDecompressor.decompress_from_url_component(b64_fragment)

        # The correct method must succeed and return non-empty data.
        result = LZStringDecompressor.decompress_from_base64(b64_fragment)
